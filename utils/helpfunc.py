...
@torch.no_grad()
def extract_vectors(net, loader, ms=[1], device=torch.device('cuda'), img_size=0, batch_size=1):
    net.eval()
    vecs = torch.zeros(img_size, net.outputdim)
    ms = [1]
    if len(ms) == 1:
        for i, input in enumerate(tqdm(loader, desc='Processing', total=len(loader))):
            vecs[i * batch_size:(i+1) * batch_size, :] = net.forward_test(input.to(device)).cpu().data.squeeze()
        ...
        for i, input in enumerate(tqdm(loader, desc='Processing', total=len(loader))):
            ...
            vec /= len(ms)
            vecs[i * batch_size:(i+1) * batch_size, :] = F.normalize(vec, p=2, dim=0)
            print('\r>>>> {}/{} done...'.format(i + 1, len(loader)), end='')
    return vecs

...
class Attention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor):
        ...
        q_token_vectors = (attn @ v)


        q = self.proj_drop(self.proj(q))


        return  q, attn, q_token_vectors
...
class Decoder(nn.Module):
    ...
    def forward(self, q, x):
        q_bn = self.bn1(q)
        cross_attn, attn_map,q_vec = self.cross_attn(q_bn, x, x)
        q = q + self.drop_path(cross_attn)
        q = q + self.drop_path(self.mlp(q))
        q_bn = self.bn2(q)
        q = q + self.drop_path(self.self_attn(q_bn, q_bn, q_bn)[0])
        q_init = self.self_attn(q_bn, q_bn, q_bn)[2]
        return q, attn_map, q_vec
...
class Token_Refine(nn.Module):
    ...
    @torch.no_grad()
    def _slice_proj_per_object(self):
        """
        proj[0].weight: [1024, num_object*mid_dim]
        객체 o에 해당하는 슬라이스 W_o: [1024, mid_dim]
        """
        lin: nn.Linear = self.proj[0]
        W = lin.weight    # [1024, num_object*mid_dim]
        W_list = []
        for o in range(self.num_object):
            s = o * self.mid_dim
            e = (o + 1) * self.mid_dim
            W_list.append(W[:, s:e].contiguous())   # [1024, mid_dim]
        return W_list  # 길이 num_object, 각 [1024, mid_dim]




    def forward(self, x: Tensor,  attn_save: bool = False, attn_root: str = None, file_name: str = None, return_aux: bool = False):
        B, _, H, W = x.size()
        ...
        for decoder in self.decoder:
            token, attns_map, q_vec = decoder(token, x)
            last_attn_map = attns_map
        token_before_proj = token
        proj_in = token_before_proj.reshape(B,-1)
        ...
        #     import matplotlib.pyplot as plt
        #     import numpy as np
        #     save_dir = os.path.join(attn_root, file_name, "flat_vectors")
        #     os.makedirs(save_dir, exist_ok=True)
        #     # 가정: attn = attention[0] → shape: [num_heads, num_tokens, 1024]
        #     B, num_heads, num_tokens, vector = q_vec.shape
        #     B, num_heads, num_tokens, spatial_size = attns_map.shape
        #     H_feat = W_feat = int(spatial_size ** 0.5)
        #     H_img, W_img = 1024, 1024

        #     attns = attns_map[0]  # [num_heads, num_tokens, 1024]

        #     attn = q_vec[0]
        #     # [num_heads, num_tokens, 1024]


        #     for token_idx in range(num_tokens):
        #         for head_idx in range(num_heads):

        #             attn_vec = attn[head_idx, token_idx]  # shape: [1024]
        #             vec_name = f"token{token_idx}_head{head_idx}.npy"
        #             np.save(os.path.join(save_dir, vec_name), attn_vec.detach().cpu().numpy())
        #             # 1. [1024] → [32, 32]
        #             attn_map = attns[head_idx, token_idx].reshape(H_feat, W_feat)
        ...
        if return_aux:
            return proj_out, {
                        "x_mem": x,                       # [B, N_k, mid_dim]
                        "last_attn_map": last_attn_map,       # [B, H, N_q, N_k]
                        "token_before_proj": token_before_proj,  # [B, N_q, mid_dim]
                    }
        return proj_out
...
class AdaFaceArc(nn.Module):
    def __init__(self, in_features, out_features, s=64.0, m=0.4, h=0.333, eps=1e-6, t_alpha=0.99):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.eps = eps
        ...
        # running statistics for norm
        self.register_buffer("batch_mean", torch.ones(1) * 20)
        self.register_buffer("batch_std", torch.ones(1) * 100)

    def forward(self, input, norm, label):
        # 1. normalize weights and inputs
        kernel_norm = F.normalize(self.weight, dim=1)  # [C, D]
        input_normed = F.normalize(input, dim=1)       # [B, D]
        cos_theta = F.linear(input_normed, kernel_norm)  # [B, C]
        cos_theta = torch.clamp(cos_theta, -1 + self.eps, 1 - self.eps)

        # 2. compute margin scaler from norm
        safe_norms = torch.clip(norm, 0.001, 100).detach()
        with torch.no_grad():
        ...
        # 3. get per-sample margin
        g_angular = -self.m * margin_scaler
        g_add = self.m + self.m * margin_scaler

        # 4. arcface angular margin
        theta = torch.acos(cos_theta)
        theta_m = torch.clip(theta + g_angular.unsqueeze(1), self.eps, math.pi - self.eps)
        cos_theta_m = torch.cos(theta_m)

        # 5. build margin matrix
        one_hot = torch.zeros_like(cos_theta)
        one_hot.scatter_(1, label.view(-1, 1), 1.0)

        logits = cos_theta * (1 - one_hot) + (cos_theta_m - g_add.unsqueeze(1)) * one_hot
...
class Token(nn.Module):
    def __init__(self, outputdim=1024, classifier_num=81313, mode='train'):
        ...
        if mode == 'train':
            self.classifier = ArcFace(in_features=self.outputdim, out_features=4389, s=math.sqrt(self.outputdim), m=0.2)

    def extract_features(self, x, attn_save: bool = False, attn_root: str = None, file_name: str = None):
        x = self.backbone(x)
        global_feature = self.tr(x, attn_save, attn_root, file_name)
        return global_feature

    def arcface_logits(self, feats, labels):
        return self.classifier(feats, labels)  # logits (B, num_classes)
...
class DELG(nn.Module):
    def __init__(self, outputdim, reduction_dim=128, classifier_num=1024,mode='train'):
    ...
    def forward(self, x, label):
        ...
        return local_loss, local_logits
